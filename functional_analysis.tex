...
\begin{nthm}[Tychonoff]\label{thm:tychonoff}
...
\begin{nthm}[Banach-Alaoglu]\label{thm:banach-alaoglu}
...
\begin{proof}~
  \begin{itemize}
    \item[1.] $\imp$ Fix a dense sequence $x_n$ in $X$. Let $\mcF = \{\hat x_n \mid n \in \N\}$. Then $\mcF$ separates the points of $X$, so $\sigma(B_{X^*}, \mcF)$ is Hausdorff and contained in the w*-topology of $B_{X^*}$. So
  ...
  \item[2.] $\imp$ By the above, $(B_{C(K)^*}, w^*)$ is metrisable. For $k \in K$, define
  \begin{align*}
    \delta_k : C(K) & \mor \K \\
    F \mapsto f(k)
  \end{align*}
  ...
  \item[2.] $\impliedby$ As $K$ is compact metrisable, it is separable. Fix a sequence $x_n$ dense in $K$. Let $f_n(x) = d(x, x_n)$. $d$ is a metric inducing the topology of $K$. Let $A$ be the subalgebra of $C(K)$ generated by $1$ and the $f_n$. Then $A$ is separable, closed under complex conjugation, separates points of $K$ and $1 \in A$. By Stone-Weierstrass, $\bar A = C(K)$. So $C(K)$ is separable.
  \item[1.] $\impliedby$ Let $K = (B_{X^*}, w^*)$. This is compact by Theorem \ref{thm:banach-alaoglu}. Since $K$ is metrisable, $C(K)$ is metrisable. It's enough to show that $X$ embeds isometrically into $C(K)$. Let
  \begin{align*}
    T : X & \mor C(K) \\
...
\begin{proof}~\\
  $\imp$ \\
  By Proposition \ref{prop:ck-separable}, $(B_{X^{**}}, w^*)$ is metrisable. Hence
  $$(B_X, w) = (B_{X^{**}}, w^*)\restriction_{B_X}$$
  is metrisable.

  $\impliedby$ \\
  Let $d$ metrise $(B_x, w)$. For all $n$, find $F_n \subseteq X^*$ finite and $\eps_n > 0$ such that
